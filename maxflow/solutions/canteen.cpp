...
void testcase() {
  ...
  int max_price = *max_element(price.begin(), price.end());
  
  for (int i=0; i<n; i++) {
    adder.add_edge(i, sink, student[i], max_price-price[i]);
  }
  ...
  if (s_flow == accumulate(student.begin(), student.end(), 0)) {
    cout << "possible ";
  } else {
    cout << "impossible ";
  }
  cout << s_flow << " ";
  cout << -cost+max_price*s_flow;
} 
